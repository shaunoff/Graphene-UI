...
class Section extends React.Component {
	...
	render() {
		...
		return (
		    ...
		    <div style={{cursor: 'pointer',display: 'flex',padding: '10px', justifyContent: 'space-between',alignItems: 'center',fontWeight: 500, color: "rgb(88, 88, 88)"}} onClick={()=>{handleClick(index)}} >
		      {title}
		      <img style={{height: "14px"}} src={`icons/${activeIndex.includes(index) ? "down" : "right"}-arrow.svg`}/>
		    </div>
					<Transition
						timeout={duration}
						...
						{(state) => {
							...
							return(
								<div ref={container => this.container = container} style={{margin: '10px', overflow: 'hidden'}}>
										{children}
						    </div>
							)
						}}
